...
write.table(dat[,c(1,2)], "gsmr_example_snps.allele", col.names=F, row.names=F, quote=F) 

# Extract the genotype data from a GWAS dataset using GCTA 
# Prepare own reference data from 1000 genomes
system("~/tools/gcta/gcta-1.94.4-linux-kernel-3-x86_64/gcta64 --bfile EUR --extract gsmr_example_snps.allele --update-ref-allele gsmr_example_snps.allele --recode --out EUR")
...
snp_id = Reduce(intersect, list(dat$SNP, snp_coeff_id))
dat_new = dat[match(snp_id, dat$SNP),]
...
#####========= Bidirectional GSMR ========#####
gsmr_results = bi_gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval, ldrho, snp_coeff_id, n_ref, heidi_outlier_flag, gwas_thresh, multi_snps_heidi_thresh, nsnps_thresh, ld_r2_thresh, ld_fdr_thresh, gsmr2_beta) 
cat("Effect of risk factor on disease: ",gsmr_results$forward_bxy) # Null because all SNPs excluded after HEIDI
