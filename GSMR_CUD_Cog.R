...
Cog_out_dat_gsmr <- read_outcome_data(
...
# Harmonization
dat_gsmr_Cog <- harmonise_data(
  exposure_dat = CUD_exp_dat_strict_new,
  outcome_dat = Cog_out_dat_gsmr
)#149 SNPs dropped for being palindromic with intermediate allele frequencies

#####======== LD Matrix ========#####
setwd('~/EA_Cannabis_NEW/GSMR')

# Save the genetic variants and effect alleles in a text file using R 
write.table(dat_gsmr_Cog[,c(1,2)], "gsmr_example_snps_Cog.allele", col.names=F, row.names=F, quote=F) 

# Extract the genotype data from a GWAS dataset using GCTA 
# Prepare own reference data from 1000 genomes
system("~/tools/gcta/gcta-1.94.4-linux-kernel-3-x86_64/gcta64 --bfile EUR --extract gsmr_example_snps_Cog.allele --update-ref-allele gsmr_example_snps_Cog.allele --recode --out EUR")
...
snp_id = Reduce(intersect, list(dat_gsmr_Cog$SNP, snp_coeff_id))
dat_new = dat_gsmr_Cog[match(snp_id, dat_gsmr_Cog$SNP),]
...
# Run GSMR
gsmr_CUD_Cog = gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval, ldrho, snp_coeff_id, n_ref, heidi_outlier_flag, gwas_thresh, single_snp_heidi_thresh, multi_snps_heidi_thresh, nsnps_thresh, ld_r2_thresh, ld_fdr_thresh, gsmr2_beta)    # GSMR analysis 
filtered_index=gsmr_CUD_Cog$used_index
cat("The estimated effect of the exposure on outcome: ",gsmr_CUD_Cog$bxy)
cat("Standard error of bxy: ",gsmr_CUD_Cog$bxy_se)
cat("P-value for bxy: ", gsmr_CUD_Cog$bxy_pval)
cat("Number of pleiotropic outliers: ", length(gsmr_CUD_Cog$pleio_snps))


#####========= Bidirectional GSMR ========#####
gsmr_CUD_Cog_bi = bi_gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval, ldrho, snp_coeff_id, n_ref, heidi_outlier_flag, gwas_thresh, multi_snps_heidi_thresh, nsnps_thresh, ld_r2_thresh, ld_fdr_thresh, gsmr2_beta) 
cat("Effect of risk factor on disease: ",gsmr_EA_Cog_bi$forward_bxy)
...
plot(bzx[filtered_index], bzy[filtered_index], pch=20, cex=0.8, bty="n", cex.axis=1.1, cex.lab=1.2,
     ...
     xlab=expression(CUD~(italic(b[zx]))),
     ylab=expression(Cognitive_EA~(italic(b[zy]))))
abline(0, gsmr_CUD_Cog$bxy, lwd=1.5, lty=2, col="dim grey")
