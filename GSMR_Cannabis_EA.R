...
EA_out_dat_gsmr <- read_outcome_data(
...
# Harmonization
dat_gsmr_1 <- harmonise_data(
  exposure_dat = CUD_exp_dat_strict_new,
  outcome_dat = EA_out_dat_gsmr
)#176 SNPs dropped for being palindromic with intermediate allele frequencies

#####======== LD Matrix ========#####
setwd('~/EA_Cannabis_NEW/GSMR')

# Save the genetic variants and effect alleles in a text file using R 
write.table(dat_gsmr_1[,c(1,2)], "gsmr_example_snps_1.allele", col.names=F, row.names=F, quote=F) 

# Extract the genotype data from a GWAS dataset using GCTA 
# Prepare own reference data from 1000 genomes
system("~/tools/gcta/gcta-1.94.4-linux-kernel-3-x86_64/gcta64 --bfile EUR --extract gsmr_example_snps_1.allele --update-ref-allele gsmr_example_snps_1.allele --recode --out EUR")
...
snp_id = Reduce(intersect, list(dat_gsmr_1$SNP, snp_coeff_id))
dat_new = dat_gsmr_1[match(snp_id, dat_gsmr_1$SNP),]
...
# Run GSMR
gsmr_CUD_EA = gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval, ldrho, snp_coeff_id, n_ref, heidi_outlier_flag, gwas_thresh, single_snp_heidi_thresh, multi_snps_heidi_thresh, nsnps_thresh, ld_r2_thresh, ld_fdr_thresh, gsmr2_beta)    # GSMR analysis 
filtered_index=gsmr_CUD_EA$used_index
cat("The estimated effect of the exposure on outcome: ",gsmr_CUD_EA$bxy)
cat("Standard error of bxy: ",gsmr_CUD_EA$bxy_se)
cat("P-value for bxy: ", gsmr_CUD_EA$bxy_pval)
cat("Number of pleiotropic outliers: ", length(gsmr_CUD_EA$pleio_snps))


#####========= Bidirectional GSMR ========#####
gsmr_CUD_EA_bi = bi_gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval, ldrho, snp_coeff_id, n_ref, heidi_outlier_flag, gwas_thresh, multi_snps_heidi_thresh, nsnps_thresh, ld_r2_thresh, ld_fdr_thresh, gsmr2_beta) 
cat("Effect of risk factor on disease: ",gsmr_EA_CUD_bi$forward_bxy)
...
abline(0, gsmr_CUD_EA$bxy, lwd=1.5, lty=2, col="dim grey")
